...
export const registerUser = async (req: Request, res: Response) => {
  try {
    let current_user = await await Model.User.findOne({
    ...
    current_user = await Model.User.create(req.body);
  ...
  try {
    if (req.params.login_by === "google") {
      try {
        ...
        let dbUsers = await Model.User.findOne({
          email: google_user.email,
          phone_number: null,
        });

        if (!dbUsers) {
          dbUsers = await Model.User.create({
            email: google_user.email,
